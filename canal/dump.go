...
func (h *dumpParseHandler) Data(db string, table string, values []string) error {
	...
	tableConfig := h.c.getTableConfig(db, table, h.c.syncconfig.DbInfo.Databases)
	if tableConfig == nil {
		return nil
	}
	...
	return h.c.eventHandler.OnRow(0, events, h.pos, h.name, nil)
}
...
func (c *Canal) tryDump() (bool, error) {
