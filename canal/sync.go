...
func (c *Canal) runSyncBinlog() error {
	var traceErr error
	defer func() {
		log.WithField("", traceErr).Error("runSyncBinlog error")
	}()
	s, err := c.startSyncer()
	if err != nil {
		traceErr = err
	...
	for {
		...
		case *replication.GTIDEvent:
			if err := c.eventHandler.OnGTIDSet(ev.GtidSet); err != nil {
				traceErr = err
				return errors.Trace(err)
			}
...
func (c *Canal) getTableConfig(dbName, tableName string, dbList []config.TableDbConfig) *config.TableConfig {
...
func (c *Canal) handleRowsEvent(e *replication.BinlogEvent, pos mysql.Position) error {
	...
	tableConfig := c.getTableConfig(schema, table, c.syncconfig.DbInfo.Databases)
	if tableConfig == nil {
		return nil
	}
	...
	return c.eventHandler.OnRow(e.Header.Timestamp, events, uint64(e.Header.LogPos), pos.Name, tableConfig)
}
